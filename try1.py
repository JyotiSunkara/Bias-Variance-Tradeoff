...
# 10 models generation
while model < 10:  
    
    linearRegressor.fit(X_train_split[model], Y_train_split[model])
    plot.scatter(X_train_split[model], Y_train_split[model], color = 'red')
    plot.plot(X_train_split[model], linearRegressor.predict(X_train_split[model]), color = 'blue')
